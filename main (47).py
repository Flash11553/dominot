...
try:
    ...
    daily_wins_collection = db["daily_wins"]
...
games = {}
TIMEOUT_MINUTES = 5
CHECK_INTERVAL_SECONDS = 60 # 1 dəqiqədən bir yoxlama (5 dəqiqəlik timeout-u dəqiq tutmaq üçün)
...
async def check_for_inactive_games():
    ...
    while True:
        ...
        try:
            ...
            # 🟢 YENİ: 'bitmiş' (finished) oyunları qısa ehtiyat müddətindən
            # (30 saniyə) sonra yaddaşdan təmizlə — bu müddət ərzində
            # poll edən bütün oyunçular artıq son nəticəni görmüş olur.
            finished_grace = datetime.timedelta(seconds=30)
            for fid, fgame in list(games.items()):
                if fgame.get("finished") and fgame.get("finished_at"):
                    if current_time - fgame["finished_at"] > finished_grace:
                        games.pop(fid, None)
            ...
            for chat_id, reason in games_to_delete:
                ...
                # 🟢 DÜZƏLİŞ: Aktiv oyun 5 dəqiqə hərəkətsiz qalsa, artıq
                # sadəcə SİLİNMİR — kimin əlində ən az daş/xal qalıbsa o
                # QALİB elan olunur (end_game_by_score ilə eyni məntiq,
                # MongoDB-yə yazılır, nəticə göstərilir).
                if reason == "active_timeout":
                    try:
                        await end_game_by_score(
                            chat_id,
                            reason_text=f"{TIMEOUT_MINUTES} dəqiqə ərzində heç kim gediş etmədi",
                            title_text=f"⏰ **VAXT BİTDİ!** {TIMEOUT_MINUTES} dəqiqə ərzində heç kim gediş etmədi."
                        )
                        logging.info(f"Oyun active_timeout səbəbi ilə (qalib elan edilərək) dayandırıldı. Chat ID: {chat_id}")
                    except Exception as e:
                        logging.error(f"active_timeout zamanı qalib elan edilə bilmədi: {e}")
                    continue
...
def _repair_unicode(text: str) -> str:
    """🟢 YENİ: Adda 'yarımçıq qalmış' (lone surrogate) simvollar varsa —
    bu, adətən əvvəlki bir yerdə addan səhvən orta simvoldan kəsilməsindən
    (məs. sənin göstərdiyin 'tofu qutuları') qaynaqlanır — bunları təmir
    edir. Sağlam adlara toxunmur."""
    try:
        text.encode("utf-8")
        return text  # artıq sağlamdır, əlavə iş lazım deyil
...
def safe_display_name(name: str) -> str:
    """Oyunçunun əsl adını OLDUĞU KİMİ saxlayır (emoji, fantaziya şrift,
    simvol — bunların heç biri silinmir/dəyişdirilmir). Yalnız ad HƏQİQƏTƏN
    boşdursa (None, boş sətir və ya sadəcə boşluqlardan ibarətdirsə) etibarlı
    bir geriqayıtma adı verir ki, bot heç vaxt xətalı/boş ad yazmasın.
    🟢 DÜZƏLİŞ: əvvəllər bəzi hallarda etibarlı (qəribə şriftli/emojili)
    adlar səhvən "Anonim"ə çevrilirdi — bu funksiya bunun qarşısını alır.
    Həmçinin köhnə/zədələnmiş adları da təmir edir (bax _repair_unicode).
    """
    if name is None:
        return "Naməlum oyunçu"
    name = _repair_unicode(name)
    stripped = name.strip()
    if not stripped:
        return "Naməlum oyunçu"
    return name
...
def update_winner_count(user_id, name):
    """Qalibin qələbə sayını MongoDB-də artırır (ümumi + günlük)."""
    ...
    # 🟢 YENİ: günlük (24 saatlıq) qələbə sayğacı — Ana səhifədəki
    # "Liderlər" bölməsi bunu istifadə edir, hər gün (UTC tarixinə görə)
    # avtomatik sıfırlanır.
    if daily_wins_collection is not None:
        today_key = datetime.datetime.utcnow().strftime("%Y-%m-%d")
        daily_wins_collection.update_one(
            {"_id": f"{user_id_str}_{today_key}"},
            {"$inc": {"wins": 1}, "$set": {"name": name, "user_id": user_id_str, "date": today_key}},
            upsert=True
        )
...
def load_top_daily_wins(limit=10):
    """Son 24 saatda (bugünkü UTC tarixi üzrə) ən çox qələbə qazanan
    oyunçuları yükləyir."""
    if daily_wins_collection is None: return []
    today_key = datetime.datetime.utcnow().strftime("%Y-%m-%d")
    return list(daily_wins_collection.find({"date": today_key}).sort("wins", -1).limit(limit))

def _player_rank_level(user_id):
    """Verilmiş istifadəçinin CARİ səviyyə/rütbəsini qaytarır (nəticə
    ekranlarında hər oyunçunun yanında göstərmək üçün)."""
    if ratings_collection is None:
        return compute_level(0)
    doc = ratings_collection.find_one({"_id": str(user_id)})
    wins = doc.get("wins", 0) if doc else 0
    return compute_level(wins)


def get_overall_rank(user_id):
    """İstifadəçinin ÜMUMİ reytinqdə neçənci yerdə olduğunu hesablayır
    (top 25-də olmasa belə, əsl sırasını göstərir)."""
    if ratings_collection is None: return None
    doc = ratings_collection.find_one({"_id": str(user_id)})
    my_wins = doc.get("wins", 0) if doc else 0
    better_count = ratings_collection.count_documents({"wins": {"$gt": my_wins}})
...
async def end_game_by_score(chat_id: int, reason_text: str = "Oyun bloklandı, heç kim gedə bilmir", title_text: str = "🚫 **OYUN BLOKANDI!** Heç kim gedə bilmir."):
    """Bütün oyunçular pass etdi / bazar boşaldı (VƏ YA 5 dəqiqə heç kim
    gediş etmədi) — ən az xallı oyunçu qalibdir.

    🟢 DÜZƏLİŞ: indi structured 'results' siyahısını da QAYTARIR ki, app
    (API) hər bitmə səbəbində (bloklanma, timeout) tam nəticəni (kimin
    əlində neçə daş/xal qaldığını) göstərə bilsin."""
    game = games.get(chat_id)
    if not game:
        return None
    ...
    winner_name = safe_display_name(players.get(winner_uid))

    for uid, name in players.items():
        record_game_played(uid, name)

    update_winner_count(winner_uid, winner_name)
    save_game_history(chat_id, winner_uid, winner_name, reason_text, players, scores)

    sorted_scores = sorted(scores.items(), key=lambda x: x[1]["score"])
    score_list = []
    results = []
    for uid, data in sorted_scores:
        name = safe_display_name(players.get(uid))
        status = "🏆 **Qalib**" if uid == winner_uid else \
                 f"{data['score']} xalı qaldı ({data['tiles']} daş)"
        score_list.append(f"*{escape_md(name)}*: {status}")
        results.append({"user_id": str(uid), "name": name, "tiles": data["tiles"], "score": data["score"], "level": _player_rank_level(uid)[0], "rank": _player_rank_level(uid)[1]})
    ...
    # 🟢 YENİ: App-da yaradılmış lobbilərin (is_app_game) arxasında əsl
    # Telegram söhbəti yoxdur — ora mesaj göndərmək cəhdi xəta yaradar.
    if not game.get("is_app_game"):
    ...
    final_info = {"winner_name": winner_name, "reason": reason_text, "results": results}
    # 🟢 DÜZƏLİŞ: dərhal silmirik — "bitmiş" kimi işarələyib qısa müddət
    # (aşağıdakı fasiləsizlik yoxlayıcısı təmizləyənə qədər) saxlayırıq ki,
    # poll edən DİGƏR oyunçular da (hərəkəti ETMƏYƏN) tam nəticəni görsün,
    # "oyun tapılmadı" xətası əvəzinə.
    game["finished"] = True
    game["finished_at"] = datetime.datetime.utcnow()
    game["final_results"] = final_info
    return final_info
...
async def handle_join_via_start(msg: types.Message, args: str):
    ...
    user_name = safe_display_name(msg.from_user.first_name)
...
async def cmd_start_welcome(msg: types.Message):
    ...
    welcome_text = (
        ...
        "🎮 <b>Oyunu Aç</b> düyməsinə basaraq tətbiqi (app) açıb daha rahat, başqa insanlarla birlikdə daha əyləncəli oynaya bilərsiniz!"
    )
    ...
    if APP_URL and "REPLACE-WITH" not in APP_URL:
        try:
            kb.add(InlineKeyboardButton(text="🎮 Oyunu Aç", web_app=types.WebAppInfo(url=APP_URL)))
        except Exception as e:
            logging.warning(f"WebApp düyməsi yaradıla bilmədi: {e}")
...
async def cmd_domino_new(msg: types.Message):
    ...
    # 🟢 YENİ: əvvəlcə HARADA oynamaq istədiyini soruş — qrupda (əvvəlki
    # kimi, stikerlərlə) və ya app-da (Mini App-ı açır).
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(InlineKeyboardButton("🎲 Qrupda Oynamaq", callback_data="domino_where_group"))
    if APP_URL and "REPLACE-WITH" not in APP_URL:
        try:
            kb.add(InlineKeyboardButton("📱 Tətbiqdə Oynamaq", web_app=types.WebAppInfo(url=APP_URL)))
        except Exception as e:
            logging.warning(f"WebApp düyməsi yaradıla bilmədi: {e}")
    await msg.reply("🎮 **Dominonu Harada Oynamaq İstəyirsiniz?**", reply_markup=kb, parse_mode="Markdown")


async def _start_group_lobby(chat_id: int):
    """🟢 Qrupda əvvəlki kimi (stikerlərlə) domino lobbisini yaradır —
    bu, HƏMİŞƏKİ qrup gedişatıdır, app-la heç qarışmır."""
    if chat_id in games:
        await bot.send_message(chat_id, "✅ Artıq aktiv domino oyunu var. Oyunu bitirmək üçün /stop yazın.")
        return
    ...
    initial_kb = get_join_keyboard(0, chat_id)
    ...
    games[chat_id]["join_msg_id"] = sent_msg.message_id


@dp.callback_query_handler(lambda c: c.data == "domino_where_group")
async def process_domino_where_group(call: types.CallbackQuery):
    await call.answer()
    try:
        await call.message.delete()
    except Exception:
        pass
    await _start_group_lobby(call.message.chat.id)
...
async def join_game_callback(call: types.CallbackQuery):
    ...
    user_name = safe_display_name(call.from_user.first_name)
...
async def process_sticker_move(msg: types.Message):
    ...
    if current_player_id != user_id:
        ...
        await bot.send_message(chat_id, f"❌ *{escape_md(safe_display_name(msg.from_user.first_name))}*, indi {escape_md(current_player_name)} adlı oyunçunun növbəsidir!", parse_mode="Markdown")
    ...
    if stone_value == "PASS":
        ...
        await bot.send_message(chat_id, f"⏩ *{escape_md(safe_display_name(msg.from_user.first_name))}* daşı olmadığı və bazar boş olduğu üçün növbəni keçdi (PASS)", parse_mode="Markdown")
        try: await msg.delete()
        except: pass
    ...
    if stone_value == "TAKE":
        ...
        await bot.send_message(chat_id, f"📥 *{escape_md(safe_display_name(msg.from_user.first_name))}* bazardan 1 daş götürdü. Bazarda **{len(game['stock'])}** daş qaldı", parse_mode="Markdown")
        try: await msg.delete()
        except: pass
    ...
    if stone_value not in game["hands"].get(user_id, []):
        await bot.send_message(chat_id, f"⚠️ *{escape_md(safe_display_name(msg.from_user.first_name))}*, bu daş əlinizdə yoxdur", parse_mode="Markdown")
        try: await msg.delete()
        except: pass
        return
...
async def process_side_choice(callback_query: types.CallbackQuery):
    ...
    a, b = map(int, stone_value.split("-"))
    user_name = safe_display_name(callback_query.from_user.first_name)
...
def _profile_payload(doc: dict) -> dict:
    ...
    return {
        "user_id": doc.get("_id"),
        "name": safe_display_name(doc.get("name")),
        "wins": wins,
...
async def api_profile(request):
    ...
    doc = ratings_collection.find_one({"_id": str(user_id)})
    payload = _profile_payload(doc) if doc else _profile_payload({"_id": str(user_id)})
    payload["overall_rank"] = get_overall_rank(user_id)
...
async def api_leaderboard_daily(request):
    """🟢 YENİ: Son 24 saatın (bugünkü) top 10 qalibi."""
    if daily_wins_collection is None:
        return web.json_response({"error": "Verilənlər bazası mövcud deyil"}, status=503)
    docs = load_top_daily_wins(10)
    out = []
    for d in docs:
        rating_doc = ratings_collection.find_one({"_id": d.get("user_id")}) if ratings_collection is not None else None
        total_wins = rating_doc.get("wins", 0) if rating_doc else 0
        level, rank = compute_level(total_wins)
        out.append({
            "user_id": d.get("user_id"),
            "name": safe_display_name(d.get("name")),
            "wins_today": d.get("wins", 0),
            "level": level,
            "rank": rank,
        })
...
async def api_history_for_user(request):
    ...
    for doc in cursor:
        out.append({
            "id": str(doc["_id"]),
            "winner_name": safe_display_name(doc.get("winner_name")),
...
async def api_history_detail(request):
    ...
    for d in details:
        d["name"] = safe_display_name(d.get("name"))
...
async def api_lobbies(request):
    """Hələ başlamamış (qeydiyyat mərhələsində olan) AÇIQ APP LOBBİLƏRİ.

    🟢 DÜZƏLİŞ: App qlobaldır — qrupda yaranan lobbilər burda GÖSTƏRİLMİR,
    yalnız app daxilində yaradılmış (is_app_game=True) lobbilər görünür.
    """
    out = []
    for chat_id, game in list(games.items()):
        if not game.get("is_app_game"):
...
async def api_config(request):
    """🟢 YENİ: App-ın t.me deep-link (Mini App) dəvət linkləri qura bilməsi
    üçün bot username-ini verir."""
...
def _game_public_state(chat_id, game, requester_id=None):
    ...
    state = {
        ...
        "creator_id": str(game.get("creator_id")) if game.get("creator_id") is not None else None,
    }
    ...
    # 🟢 DÜZƏLİŞ: oyun artıq bitmişdirsə (amma hələ 'games' dict-dən
    # silinməyibsə — qısa bir ehtiyat müddəti üçün saxlanılır), tam nəticəni
    # də əlavə et ki, hərəkəti ETMƏYƏN digər oyunçular da (sadəcə poll edən)
    # "oyun tapılmadı" xətası ƏVƏZİNƏ əsl nəticəni görsün.
    if game.get("finished"):
        state["finished"] = True
        state["final_results"] = game.get("final_results")
    return state
...
async def api_lobby_join(request):
    ...
    user_id, user_name = auth["id"], safe_display_name(auth["first_name"])
    ...
    if game.get("is_app_game") and _user_in_any_app_game(user_id):
        return web.json_response({"error": "Siz artıq başqa bir lobbinin/oyunun üzvüsünüz. Əvvəlcə ondan çıxın."}, status=409)
    ...
    # 🟢 YENİ: yalnız QRUP lobbiləri üçün Telegram-a bildiriş göndər —
    # app daxilində yaradılmış lobbinin arxasında əsl Telegram söhbəti yoxdur.
    if not game.get("is_app_game"):
        await update_join_message(chat_id)
        count = len(game["players"])
        try:
            await bot.send_message(
                chat_id=chat_id,
                text=f"✅ **{escape_md(user_name)}** (app-dan) oyuna qoşuldu! ({count}/4 nəfər)",
                parse_mode="Markdown"
            )
        except Exception as e:
...
def _user_in_any_app_game(user_id):
    """Bir istifadəçi eyni anda YALNIZ bir app-lobbisinin/oyununun üzvü ola bilər.
    🟢 DÜZƏLİŞ: artıq BİTMİŞ (finished) oyunlar bura hesablanmır — əks halda
    30 saniyəlik təmizlənmə pəncərəsində istifadəçi yanlışlıqla "başqa
    oyundasan" xətası ala bilərdi."""
    for g in games.values():
        if g.get("is_app_game") and not g.get("finished") and user_id in g.get("players", {}):
            return True
    return False


async def api_lobby_create(request):
    """🟢 YENİ: App daxilində, heç bir Telegram qrupuna bağlı olmayan yeni
    lobbi yaradır — qlobal app məntiqi buradan başlayır."""
    auth = _auth_from_request(request)
    if not auth:
        return web.json_response({"error": "Kimlik doğrulanmadı"}, status=401)
    user_id, user_name = auth["id"], safe_display_name(auth["first_name"])

    if _user_in_any_app_game(user_id):
        return web.json_response({"error": "Siz artıq başqa bir lobbinin/oyunun üzvüsünüz. Əvvəlcə ondan çıxın."}, status=409)

    lobby_id = random.randint(10**9, 2 * 10**9)
    while lobby_id in games:
        lobby_id = random.randint(10**9, 2 * 10**9)
    ...
    return web.json_response({"ok": True, "chat_id": lobby_id})


async def api_lobby_leave(request):
    """🟢 YENİ: Lobbidən çıxma. Qurucu çıxsa lobbi tamamilə dağılır,
    digər oyunçular çıxsa yalnız özləri silinir. Oyun başladıqdan sonra
    çıxmaq mümkün deyil."""
    try:
        chat_id = int(request.match_info["chat_id"])
    except ValueError:
    ...
    game = games.get(chat_id)
    if not game or not game.get("is_app_game"):
        return web.json_response({"error": "Lobbi tapılmadı"}, status=404)
    if user_id not in game["players"]:
        return web.json_response({"error": "Bu lobbinin üzvü deyilsiniz"}, status=403)
    if game.get("turn") is not None:
        return web.json_response({"error": "Oyun artıq başlayıb, çıxa bilməzsiniz"}, status=409)

    if game.get("creator_id") == user_id:
        games.pop(chat_id, None)
        return web.json_response({"ok": True, "disbanded": True})
    else:
        game["players"].pop(user_id, None)
        return web.json_response({"ok": True, "disbanded": False})


async def api_leave_all(request):
    """🟢 YENİ: 'Bütün Oyunlardan Çıx' təhlükəsizlik düyməsi — istifadəçini
    HANSI app lobbisində/oyununda olursa olsun tamamilə çıxarır. Əgər
    (əvvəlki test sessiyalarından və s.) yaddaşda unudulmuş qeyd qalıbsa,
    bu, istifadəçini onlardan həmişəlik xilas edir."""
    auth = _auth_from_request(request)
    if not auth:
        return web.json_response({"error": "Kimlik doğrulanmadı"}, status=401)
    user_id = auth["id"]
    ...
    for chat_id, game in list(games.items()):
        ...
        if user_id not in game.get("players", {}):
            continue
        if game.get("creator_id") == user_id or len(game["players"]) <= 1:
            games.pop(chat_id, None)
        else:
            game["players"].pop(user_id, None)
            game.get("hands", {}).pop(user_id, None)
            # Növbə indeksini təhlükəsiz saxla (oyunçu sayı azaldısa)
            if game.get("turn") is not None and game["turn"] >= len(game["players"]):
                game["turn"] = 0
        removed_from += 1

    return web.json_response({"ok": True, "removed_from": removed_from})


async def api_lobby_start(request):
    """🟢 YENİ: App-da yaradılmış lobbini başladır — botun 'Oyunu Başlat'
    məntiqi ilə HƏRFİ EYNİ (daş sayı, paylama qaydası)."""
    try:
        chat_id = int(request.match_info["chat_id"])
    except ValueError:
        return web.json_response({"error": "Yanlış chat_id"}, status=400)
    ...
    if user_id not in game["players"]:
        return web.json_response({"error": "Bu lobbinin üzvü deyilsiniz"}, status=403)
    if game.get("turn") is not None:
        return web.json_response({"error": "Oyun artıq başlayıb"}, status=409)
    if len(game["players"]) < 2:
        return web.json_response({"error": "Oyunçular çatışmır (min 2 nəfər)"}, status=409)

    all_stones = list(DOMINO.keys())
    random.shuffle(all_stones)
    stones_per_player = 7
    remaining = all_stones[:]
    for p in game["players"]:
        game["hands"][p] = remaining[:stones_per_player]
        remaining = remaining[stones_per_player:]
    game["stock"] = remaining
    game["turn"] = 0
    game["left"] = None
    game["right"] = None
    game["board_tiles"] = []
...
async def api_game_play(request):
    ...
    if len(game["hands"][user_id]) == 0:
        ...
        results = []
        for uid, data in sorted_scores:
            name = safe_display_name(game['players'].get(uid))
            status = "🏆 **Qalib**" if uid == user_id else f"{data['score']} xalı qaldı ({data['tiles']} daş)"
            score_list.append(f"*{escape_md(name)}*: {status}")
            results.append({"user_id": str(uid), "name": name, "tiles": data["tiles"], "score": data["score"], "level": _player_rank_level(uid)[0], "rank": _player_rank_level(uid)[1]})
        result_message = f"🎉 **OYUN BAŞA ÇATDI!** 🎉\n\n🏆 **{escape_md(user_name)}** qazandı!\n\n" + "\n".join(score_list)
        ...
        game["finished"] = True
        game["finished_at"] = datetime.datetime.utcnow()
        game["final_results"] = {"winner_name": user_name, "reason": "Bütün daşlarını atdı", "results": results}
        return web.json_response({"ok": True, "game_over": True, "winner": user_name, "reason": "Bütün daşlarını atdı", "results": results})

    game["turn"] = (game["turn"] + 1) % len(game["players"])

    if check_for_blocked_game(chat_id):
        end_info = await end_game_by_score(chat_id)
        return web.json_response({"ok": True, "game_over": True, **(end_info or {})})

    # 🟢 SİNXRONİZASİYA: yalnız QRUP oyunlarında qrupa da (foto+mətn) göndər
    if not game.get("is_app_game"):
        await send_board_status(chat_id)
...
async def api_game_leave(request):
    """🟢 YENİ: Oyun DAVAM edərkən bir oyunçu 'Dəqiq çıxmaq istəyirsən?'
    təsdiqindən sonra oyundan çıxır. Onun əlindəki daşlar sadəcə BATIR
    (bazara/digərlərinə verilmir). 2 nəfərlikdə qalan tək oyunçu dərhal
    qalib olur; 3-4 nəfərlikdə oyun növbə pozulmadan davam edir və çıxan
    oyunçu artıq masada göstərilmir."""
    try:
        chat_id = int(request.match_info["chat_id"])
    except ValueError:
        return web.json_response({"error": "Yanlış chat_id"}, status=400)
    ...
    game = games.get(chat_id)
    if not game or game.get("turn") is None:
        return web.json_response({"error": "Aktiv oyun tapılmadı"}, status=404)
    if user_id not in game.get("players", {}):
        return web.json_response({"error": "Bu oyunun üzvü deyilsiniz"}, status=403)

    player_ids_before = list(game["players"].keys())
    leaving_index = player_ids_before.index(user_id)
    current_turn_idx = game.get("turn", 0)

    # Oyunçunu çıxar — əlindəki daşlar sadəcə batır, heç kimə verilmir.
    game["players"].pop(user_id, None)
    game["hands"].pop(user_id, None)

    remaining = list(game["players"].keys())

    if len(remaining) <= 1:
        # Yalnız 1 nəfər qaldı — o, dərhal qalib elan olunur.
        if remaining:
            winner_uid = remaining[0]
            winner_name = safe_display_name(game["players"].get(winner_uid))
            scores = calculate_scores(game)
            for uid, name in game["players"].items():
                record_game_played(uid, name)
            update_winner_count(winner_uid, winner_name)
            reason_text = f"{user_name} oyundan çıxdı"
            save_game_history(chat_id, winner_uid, winner_name, reason_text, game["players"], scores)
            results = []
            for uid in remaining:
                data = scores.get(uid, {"score": 0, "tiles": 0})
                lvl, rnk = _player_rank_level(uid)
                results.append({"user_id": str(uid), "name": winner_name, "tiles": data["tiles"], "score": data["score"], "level": lvl, "rank": rnk})
            board_msg_ids.pop(chat_id, None)
            game["finished"] = True
            game["finished_at"] = datetime.datetime.utcnow()
            game["final_results"] = {"winner_name": winner_name, "reason": reason_text, "results": results}
        else:
            games.pop(chat_id, None)
        return web.json_response({"ok": True, "left": True})

    # 3-4 nəfər qalıb — növbəni düzgün saxlayaraq davam et.
    if leaving_index < current_turn_idx:
        new_turn = current_turn_idx - 1
    else:
        new_turn = current_turn_idx
    game["turn"] = new_turn % len(remaining)
    game["last_activity"] = datetime.datetime.utcnow()

    if check_for_blocked_game(chat_id):
        await end_game_by_score(chat_id)
        return web.json_response({"ok": True, "left": True, "game_over": True})
...
async def start_api_server():
    ...
    app.router.add_get("/api/leaderboard/daily", api_leaderboard_daily)
    ...
    app.router.add_post("/api/lobby/create", api_lobby_create)
    app.router.add_post("/api/lobby/{chat_id}/start", api_lobby_start)
    app.router.add_post("/api/lobby/{chat_id}/leave", api_lobby_leave)
    app.router.add_post("/api/user/leave-all", api_leave_all)
    ...
    app.router.add_post("/api/game/{chat_id}/leave", api_game_leave)
